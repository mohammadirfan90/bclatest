-- =============================================================================
-- Migration 008: Add Idempotency to Teller Procedures
-- Implements idempotency protection for sp_teller_deposit and sp_teller_withdraw
-- =============================================================================

DELIMITER //

-- =============================================================================
-- PROCEDURE: sp_teller_deposit (with idempotency)
-- Deposits cash into customer account using double-entry with idempotency protection
-- =============================================================================
...
BEGIN
    ...
    -- Only proceed if not an idempotent replay
        ...
        ELSE
            ...
            ELSE
                ...
                ELSE
                    ...
                    -- 10. Insert DEBIT ledger entry (Bank Cash - money out)
                    ...
                    -- 11. Insert CREDIT ledger entry (Customer - money in)
                    INSERT INTO ledger_entries (
                        transaction_id,
                        account_id,
                        entry_type,
                    ...
                    -- 12. Update materialized balances
                    ...
                    -- 16. Store idempotency result (if key provided)
                    IF p_idempotency_key IS NOT NULL AND p_idempotency_key != '' THEN
...
-- =============================================================================
-- PROCEDURE: sp_teller_withdraw (with idempotency)
-- Withdraws cash from customer account using double-entry with idempotency protection
-- =============================================================================
    ...
    IF p_message != 'Idempotent replay' THEN
        ...
        ELSE
            ...
            IF v_cash_account_id IS NULL THEN
                ...
                ROLLBACK;
            ELSE
                -- 4. Lock and validate customer account first
                SELECT ab.available_balance, a.status, a.customer_id
                INTO v_customer_balance, v_account_status, v_customer_id
                FROM account_balances ab
                INNER JOIN accounts a ON a.id = ab.account_id
                ...
                ELSE
                    ...
                    ELSE
                        ...
                        -- 10. Insert DEBIT ledger entry (Customer - money out)
                        INSERT INTO ledger_entries (
                            transaction_id,
                            account_id,
                            entry_type,
                        ...
                        -- 11. Insert CREDIT ledger entry (Bank Cash - money in)
                        INSERT INTO ledger_entries (
                            transaction_id,
                            account_id,
                            entry_type,
